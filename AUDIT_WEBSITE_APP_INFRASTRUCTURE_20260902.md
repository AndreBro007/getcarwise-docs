...
| CarClever Guide | 1053 | Repoint to Find My Car once ready | **Verified via Chrome extension + WP REST API** (`wp-json/wp/v2/pages/1053`), bypassing the Jetpack-gated MCP connector. Last modified Aug 30, 2026 (after Find My Car's Aug 25 submission). Content already describes new+used scope ("3.4 million active dealer listings covering new, used, certified pre-owned, and demo inventory") and explicitly documents Claude setup ("Open Settings → Connectors in Claude on web...") alongside ChatGPT setup. **But the only outbound link on the page is to the ChatGPT app** (`chatgpt.com/apps/carclever/...`) — no link to Find My Car or any Claude entry point, despite the page's own text walking through Claude setup steps. Copy appears to already assume new+used / multi-assistant scope; the gap is purely a missing link, not stale copy like Try CarClever.

## Bottom line (facts only)

All four web-app tools (Lite, Deal Score, Price Check, VIN Check) run on `carclever-widget`'s own Vercel deployment. **All 8 of 8 audited pages** now confirmed: none link to Find My Car or provide a Claude entry point anywhere on the site — including CarClever Guide (1053), whose own body text already documents Claude setup instructions but has no Claude link anywhere on the page. The homepage headline has already changed since the Aug 26 audit in a way that document didn't anticipate — worth checking with André whether that edit was intentional/who made it.

**Access note:** CarClever Guide (1053) was verified using the Claude-in-Chrome extension, navigating to the WordPress REST API endpoint directly (`getcarwise.app/wp-json/wp/v2/pages/{id}`) in an authenticated browser tab. This bypassed both the Jetpack-gated MCP connector and the earlier web_search+web_fetch dead end. Same method available for future WP investigation without waiting on a Jetpack plan upgrade.
