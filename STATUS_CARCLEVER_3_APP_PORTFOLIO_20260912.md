...
3. **Freeze V2 during review** unless a platform requests a correction or a concrete reproducible defect appears. **Deliberate, recorded exception (Sep 17), covers BOTH Anthropic and OpenAI reviews:** the Edmunds CJ→Impact affiliate-link swap in `lib/edmunds-cj.ts` is being made against this branch despite the freeze — André's explicit decision, reasoning: (a) the Anthropic review may already be "in motion" via the open Marco/support MCP-URL-change thread, so this doesn't introduce a new disturbance where none existed; (b) end-of-September is a hard external deadline independent of review timelines, and applies equally to OpenAI's live Sep 12 resubmission; (c) the edit has zero schema/tool-description/CSP footprint (confirmed by code inspection — this app has no CSP config at all), making it structurally invisible to any automated review scan on either platform. Since both platforms build from the same shared branch/codebase (two separate Vercel projects — `carclever-find-my-car` for Anthropic, `ccfmc-dev-v2` for OpenAI — both auto-build every commit), the code change is unavoidably shared; the decision made here is to promote to production on **both platforms together**, not staggered. Full detail: `STRATEGY_EDMUNDS_IMPACT_MIGRATION_20260916.md`. Change is built, tested (79/79 test suite pass, confirmed clean build on both Vercel projects — one transient build error on an early intermediate commit was found and confirmed already resolved by a later commit), on a branch off the exact reviewed SHA, before any production-promotion decision is made separately.
