...
**Revised evidence method:** First use any existing server logs or provider request records to classify known owner tests, background/probe traffic, direct WordPress use and unclassified production calls, without adding production instrumentation. If existing records cannot separate them, arrange a short agreed quiet period with no discretionary development/test calls while live surfaces remain unchanged, and read daily dashboard deltas plus endpoint health. This estimates *routine combined live consumption* under that behavior, not unique users or future approval traffic. An isolated QA/prod key or new logging would be a later production change requiring approval. If a quiet window is impractical or unclassified volume remains material, label Free capacity unproven and plan explicit testing discipline/cap alerts before any downgrade.
...
2. **Auto.dev routine usage (existing logs or agreed quiet window):** classify development/test, background probe and live calls where evidence permits; measure the combined daily delta without discretionary tests, then project monthly demand and headroom against Free's shared cap. Do not use the mixed 14,965/10,987 dashboard totals as user demand. Confirm whether blocked Growth/probe calls consume quota, whether unguarded Specs is fixed, and how future QA calls would be constrained on Free.
...
**Decision rule:** Choose the cheapest configuration whose live paths still pass functional, truthful-output and quota checks. If **routine combined live plus planned QA** usage projects comfortably below 700 calls/month on a credible separated/quiet baseline, with guards and regression passing, a reversible Auto.dev downgrade can be proposed for approval. At 700–1,000 or unknown failed-call semantics, resolve avoidable calls or retain Growth temporarily. Above 1,000, decide between a time-bound paid renewal supported by measured value and a deliberate retirement; never silently break live paths. No downgrade is authorized here.
...
**Immediate next input:** Fractal plan/renewal and true free hosted-production terms, plus agreement on whether an existing-log classification or a short no-discretionary-test observation window is possible. The Auto.dev screenshot establishes mixed volume and plan, but does not establish user volume. Controlled Free regression follows the routine-usage measurement and a reviewable rollback plan.